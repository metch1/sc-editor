...
SC Editor now supports exporting animations directly from the command line.

Export a specific animation:

```shell
java -jar sc-editor.jar --export /path/to/your/file.sc --name zuzu_004_winLoop
```

List available export names inside a `.sc` file:

```shell
java -jar sc-editor.jar --export /path/to/your/file.sc
```
